#!/usr/bin/env pwsh
# Fully automated GitHub Pages deployment using GitHub CLI
# No user input required - gets GitHub username automatically from gh CLI
...
try {
    ...
    if (-not $repoExists) {
        # Create the repo
        gh repo create $RepoName --public --description "aymanaboghonim/DevOps Visions landing page clean implementation"
        if ($LASTEXITCODE -ne 0) {
